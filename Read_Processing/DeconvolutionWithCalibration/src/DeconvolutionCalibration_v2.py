...
This is used for Deconvolution analysis in the manuscript.
...
--name: name of output files
    ...
    name_Deconvolution_Calibration_R1.fq: Deconvoluted Read1 with Non-methy C convertd back and sequencing quality calibrated for mismatch bases
...
Note:
(1) Use multiple thread (nodes=ncpu) to speed up.
(2) tempory dir is named based on time, could be called by different runs.
(3) See comparison of using different Deconvolution strategies for SNP calling in SummaryOfResult.xlsx Deconvolutino_SNP.
'''
...
def CheckFormat(file):
    '''
    Detech whether a file is compressed or not,
    return True for gz and False for non-compressed file
    '''
    command = 'gzip -l {} > /dev/null'.format(file)
    try:
...
def main(Read1, Read2, dic_probability, name):
    ...
    with open(os.path.join(CWD, '{}_Deconvolution_Calibration_R1.fq'.format(name)), 'w') as output:
    ...
    print os.path.join(CWD, '{}_Deconvolution_Calibration_R1.fq'.format(name))

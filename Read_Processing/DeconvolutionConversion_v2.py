...
For methyl-SNP-Seq, run Deconvolution without Calibration, e.g. for sequencing microbiome whose genome is not known.
...
Logic:
    Criteria for report read in  Deconvolution_R1.fq:
    ...
    Convert read by comparing R1 with R2:
    If R1!=N and R2!=N:
        if R1==R2:
...
Usage:
$python DeconvolutionConversion_v2.py --Read1 AccRep1_R1_val_1.fq --Read2 AccRep1_R2_val_2.fq --name AccRep1
...
--name: name of output files
    ...
    name_DeconvolutedRead.fq: Deconvoluted Read1 with Non-methy C convertd back and sequencing quality calibrated for mismatch bases
    name.Deconvolution.5mC: C or 5mC report

Note:
(1) Do not use multiple processing.
'''
...
class Deconvolute:
    ...
    def DeconvoluteFastq(self):
        ...
        print "Deconvolution of Read1 without BaseQuality Calibration."
